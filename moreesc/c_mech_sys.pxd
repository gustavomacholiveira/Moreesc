...
cdef np.float_t c_get_opening(np.ndarray X)

cpdef np.ndarray set_opening(np.ndarray X, np.float_t h)

cdef np.float_t c_get_velocity(np.ndarray X, np.ndarray an)
