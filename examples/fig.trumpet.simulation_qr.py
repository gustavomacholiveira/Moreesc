...
if False:
    # Plot spectrogram
    Figs = sim.trace(fmax=5000., trace_signals=0, trace_spectrums=1)
...
#                    arrowprops={'arrowstyle': "-", 'alpha': 0.})
        if el.get('axvline', False):
            ...
            axs.set_xticklabels([r'$f_{ac}^{%d}$' % i for i in range(idx_maxp1)])
        if el.get('axhline', False):
            axs.set_yticks(fn[:idx_maxp1])
            axs.set_yticklabels([r'$f_{ac}^{%d}$' % i for i in range(idx_maxp1)])
